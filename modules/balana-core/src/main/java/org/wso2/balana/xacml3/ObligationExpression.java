...
public class ObligationExpression extends AbstractObligation {
	...
	public static ObligationExpression getInstance(Node root,
		...
		URI obligationId;
		String obligationType = new String();
		int fulfillOn;
		String effect;
		...
		try {
			if (nodeAttributes.getNamedItem("ObligationType") != null) {
				obligationType = nodeAttributes.getNamedItem("ObligationType")
						.getNodeValue();
			}
		} catch (Exception e) {
			throw new ParsingException(
					"Error parsing required ObligationType in "
							+ "ObligationExpressionType", e);
		}
	...
	public ObligationResult evaluate(EvaluationCtx ctx) {
		...
		return new Obligation(assignments, obligationId, obligationType);
	}

	/**
	 * Encodes this <code>ObligationExpression</code> into its XML form and
	 * writes this out to the provided <code>StringBuilder<code>
	 *
	 * @param builder
	...
	public void encode(StringBuilder builder) {
		...
		builder.append("<ObligationExpression ObligationId=\"")
				.append(obligationId.toString()).append("\" FulfillOn=\"")
				.append(AbstractResult.DECISIONS[fulfillOn]).append("\">\n");
